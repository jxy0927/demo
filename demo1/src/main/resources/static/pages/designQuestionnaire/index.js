...
const  fechCountNumber = () => {
  let number =$util.getPageParam('countNumber')
...
const handleImportQuestion = () =>{
  ...
  fileInput.addEventListener('change', handleImportQuestions);
  fileInput.click();
}
...
const handleImportQuestions = (event) => {
  ...
  reader.onload = (event) => {
    ...
    questions.forEach(question => {
      ...
      const problemIndex = problem.length - 1;
      problem[problemIndex].problemName = question.problemName;
      problem[problemIndex].mustAnswer = question.mustAnswer;
      problem[problemIndex].option = question.option;
      $(`#question${problemIndex} #problemName`).val(question.problemName);
      ...
      question.option.forEach((option, optionIndex) => {
        $(`#question${problemIndex} #optionItem${optionIndex} #chooseTerm`).val(option.chooseTerm);
      });
      switch (question.type) {
        case 1:
          singleChoiceEditFinish(problemIndex);
          break;
        case 2:
          multipleChoiceEditFinish(problemIndex);
          break;
...
const handleEditFinish = () => {
  ...
  // let designQuestionnaire= $util.getPageParam('createNewQuestionnaire')

  let  designQuestionnaire= {
    projectId:$util.getPageParam('createNewQuestionnaire').projectId,
    questionnaireType: $util.getPageParam('createNewQuestionnaire').questionnaireType,
    questionnaireName:$util.getPageParam('createNewQuestionnaire').questionnaireName,
    questionnaireContent:$util.getPageParam('createNewQuestionnaire').questionnaireContent,
    startTime:$util.getPageParam('createNewQuestionnaire').startTime,
    stopTime:$util.getPageParam('createNewQuestionnaire').stopTime,
    projectName:$util.getPageParam('createNewQuestionnaire').projectName,
  }
  ...
  $.ajax({
    ...
    type: "POST",
    data: JSON.stringify(designQuestionnaire),
    dataType: "json",
